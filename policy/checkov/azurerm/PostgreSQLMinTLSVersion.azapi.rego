...
valid_azapi_postgresql_server_min_tls_version(resource) if {
    resource.body.properties.minimalTlsVersion == "TLS1_2"
}
...
deny_CKV_AZURE_147 contains reason if {
    ...
    not valid_azapi_postgresql_server_min_tls_version(resource)

    reason := sprintf("checkov/CKV_AZURE_147: Ensure PostgreSQL is using the latest version of TLS encryption. Resource %s must have minimalTlsVersion set to TLS1_2. https://github.com/bridgecrewio/checkov/blob/main/checkov/terraform/checks/resource/azure/PostgreSQLMinTLSVersion.py", [resource.address])
}
