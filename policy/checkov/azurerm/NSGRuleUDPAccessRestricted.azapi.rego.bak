...
# azapi
valid_azapi_nsg_rule_udp_access_restricted(resource) if {
    not is_azapi_nsg_rule_allowing_udp_from_internet(resource)
}

is_azapi_nsg_rule_allowing_udp_from_internet(resource) if {
    rule := resource.body.properties.securityRules[_].properties
    lower(rule.protocol) == "udp"
    lower(rule.direction) == "inbound"
    lower(rule.access) == "allow"
    contains(INTERNET_ADDRESSES, lower(rule.sourceAddressPrefix))
}

deny_CKV_AZURE_77 contains reason if {
    resource := data.utils.resource(input, "azapi_resource")[_]
    resource.type == "Microsoft.Network/networkSecurityGroups/2024-05-01"
    not valid_azapi_nsg_rule_udp_access_restricted(resource)
    reason := sprintf("checkov/CKV_AZURE_77: Ensure that UDP Services are restricted from the Internet in azurerm_network_security_group '%s'. https://github.com/bridgecrewio/checkov/blob/main/checkov/terraform/checks/resource/azure/NSGRuleUDPAccessRestricted.py", [resource.address])
}
