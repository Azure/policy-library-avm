...
valid_azapi_service_fabric_cluster_protection_level(resource) if {
    properties := resource.body.properties
    fabric_settings := properties.fabricSettings
    some(setting; fabric_settings)
    setting.name == ["Security"]
    params := setting.parameters[0]
    params.name == "ClusterProtectionLevel"
...
deny_CKV_AZURE_125 contains reason if {
    ...
    not valid_azapi_service_fabric_cluster_protection_level(resource)
