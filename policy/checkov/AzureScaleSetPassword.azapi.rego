...
deny_CKV_AZURE_49 contains reason if {
    ...
    reason := sprintf("checkov/CKV_AZURE_49: Ensure Azure linux scale set does not use basic authentication(Use SSH Key Instead). %s https://github.com/bridgecrewio/checkov/blob/main/checkov/terraform/checks/resource/azure/AzureScaleSetPassword.py", [resource.address])
}
