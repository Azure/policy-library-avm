...
deny_AVM_SEC_AZPOLICY_BUILTIN_1 contains reason if {
	...
	reason := sprintf("avmsec/AVM_SEC_AZPOLICY_BUILTIN_1: Firewall rules should be defined on your Azure Cosmos DB accounts to prevent traffic from unauthorized sources. Accounts that have at least one IP rule defined with the virtual network filter enabled are deemed compliant. Accounts disabling public access are also deemed compliant %s: https://github.com/Azure/policy-library-avm/blob/main/policy/avmsec/Cosmos_NetworkRulesExist.azapi.rego", [res.address])
}
...
AVM_SEC_AZPOLICY_BUILTIN_1_publicNetworkAccessEnabledOrOmitted(r) if {
	not r.values.body.properties.publicNetworkAccess == r.values.body.properties.publicNetworkAccess
}

AVM_SEC_AZPOLICY_BUILTIN_1_publicNetworkAccessEnabledOrOmitted(r) if {
	r.values.body.properties.publicNetworkAccess == "Enabled"
}

AVM_SEC_AZPOLICY_BUILTIN_1_isVritualNetworkFilteredDisabledOrOmitted(r) if {
	not r.values.body.properties.isVirtualNetworkFilterEnabled == r.values.body.properties.isVirtualNetworkFilterEnabled
}

AVM_SEC_AZPOLICY_BUILTIN_1_isVritualNetworkFilteredDisabledOrOmitted(r) if {
	r.values.body.properties.isVirtualNetworkFilterEnabled == false
}

AVM_SEC_AZPOLICY_BUILTIN_1_no_ip_rules(r) if {
	not r.values.body.properties.ipRules == r.values.body.properties.ipRules
}

AVM_SEC_AZPOLICY_BUILTIN_1_no_ip_rules(r) if {
	count({x | x := r.values.body.properties.ipRules[_]}) == 0
}

AVM_SEC_AZPOLICY_BUILTIN_1_no_ip_range_filter(r) if {
	not r.values.body.properties.ipRangeFilter == r.values.body.properties.ipRangeFilter
}

AVM_SEC_AZPOLICY_BUILTIN_1_no_ip_range_filter(r) if {
	r.values.body.properties.ipRangeFilter == ""
}
...
AVM_SEC_AZPOLICY_BUILTIN_1_no_private_endpoint_connections(r) if {
	count({x | x := r.values.body.properties.privateEndpointConnections[_]; AVM_SEC_AZPOLICY_BUILTIN_1_approved(r, x)}) < 1
}
